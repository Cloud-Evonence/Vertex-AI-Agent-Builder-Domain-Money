...
try:
    genai.configure(api_key=os.getenv("GEMINI_API_KEY", "AIzaSyBmbq4W9G7tk-64zI"))
...
async def process_file(files: list[UploadFile] = File(...)):
    ...
    # Simulate google.colab.files.upload()
    class MockFiles:
        @staticmethod
        def upload():
            return uploaded
    builtins.files = MockFiles

    try:
        await main()
        return JSONResponse(content={"status": "Processing complete"}, status_code=200)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Optional: run main() manually if "manual" argument passed
